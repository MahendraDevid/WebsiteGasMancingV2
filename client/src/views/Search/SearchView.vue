...
const fetchResults = async (query) => {
    ...
    try {
        ...
        if (Object.keys(params).length === 0)
            response = await api.getAllPlaces()
        else
            response = await api.searchPlaces(params)
        ...
        <section class="search-section">
            <div class="search-container-custom">
                ...
                <div class="search-field-custom">
                    ...
                    <input type="text" class="search-input-custom" v-model="searchKeyword"
                        placeholder="Mau mancing dimana?" @keyup.enter="handleSearch" />
                </div>

                <!-- Tanggal -->
                <div class="search-field-custom">
                    <img src="/img/calendar.png" class="search-icon" alt="Tanggal">
                    <input class="search-input-custom" :type="searchDate ? 'date' : 'text'"
                        placeholder="Tanggal Mancing" v-model="searchDate" @focus="($event.target.type = 'date')"
